...
class LanguageModel():
        ...
        def __init__(self, dumped_words):
                ...
                self.train = np.reshape(self.train_array, (len(self.train_array), self.SEQUENCE_LENGTH, 1))
                self.train = self.train/float(len(self.word_map))
                self.target = np_utils.to_categorical(self.target_array)      

                self.model = Sequential()
                self.model.add(LSTM(256, input_shape=(self.train.shape[1], self.train.shape[2]), return_sequences=True))
        ...
        def train_model(self):
                ...
                self.model.fit(self.train, self.target, epochs=50, batch_size=64, callbacks=callback_list)
        ...
        def predict_word(self, query_text, words_wanted=1):
                ...
                pattern = list(np.ones(self.SEQUENCE_LENGTH-len(query_int)).astype(int)) + query_int
